...
def scenario_pkl_plot():
    ...
    for robot_id in range(SceneSetup.robot_num):
        ...
        # PLOT THE Circular Gain
        # ---------------------------------------------------¨
        fig, ax = plt.subplots(1, figsize=(6.4, 2))
        # plt.rcParams.update({'font.size': FS})
        plt.rcParams['text.usetex'] = True
        # plot
        plot_pickle_individual_id(ax, __stored_data, __end_idx, 'k_cir_'+str(robot_id))
        # label
        ax.set(ylim= (-1.1, 1.1))
        ax.set_xlabel(r'$t$ [s]', fontsize=14)
        ax.set_ylabel(r'$k_{cir}$', fontsize=14 )
        ax.legend(loc= 'best', prop={'size': leg_size})
        #plt.show()
        figname = SimSetup.sim_defname+str(robot_id)+'_k_cir.pdf'
        pngname = SimSetup.sim_defname+str(robot_id)+'_k_cir.png'
        plt.savefig(figname, bbox_inches="tight", dpi=300)
        plt.savefig(pngname, bbox_inches="tight", dpi=300)
        print( 'export figure: ' + figname, flush=True)

        plt.close('all')
        # PLOT THE Circular Gain k_sts
        # ---------------------------------------------------¨
        fig, ax = plt.subplots(1, figsize=(6.4, 2))
        # plt.rcParams.update({'font.size': FS})
        plt.rcParams['text.usetex'] = True
        # plot
        plot_pickle_individual_id(ax, __stored_data, __end_idx, 'k_sts_'+str(robot_id))
        # label
        ax.set(ylim= (-.15, 0.1))
        ax.set_xlabel(r'$t$ [s]', fontsize=14)
        ax.set_ylabel(r'$k_{sts}$', fontsize=14 )
        ax.legend(loc= 'best', prop={'size': leg_size})
        #plt.show()
        figname = SimSetup.sim_defname+str(robot_id)+'_k_sts.pdf'
        pngname = SimSetup.sim_defname+str(robot_id)+'_k_sts.png'
        plt.savefig(figname, bbox_inches="tight", dpi=300)
        plt.savefig(pngname, bbox_inches="tight", dpi=300)
        print( 'export figure: ' + figname, flush=True)
...
def exp_video_pkl_plot():
    ...
    time_snap = [0, 11, 19, 27]  # in seconds
